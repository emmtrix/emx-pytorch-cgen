...
class Parser:
    ...
    def _validate_example_inputs(
        ...
        if not tensor_examples:
            if all_tensor_examples:
                raise CodegenBackendError(
                    "codegen backend supports only torch.float32, torch.int8, torch.uint8, torch.int32, or torch.bool tensors"
                )
            return None
    ...
    def _infer_empty_strided_dtype(
        ...
        if dtype_info is None:
            raise CodegenBackendError(
                "codegen empty_strided supports only torch.float32, torch.int8, torch.uint8, torch.int32, or torch.bool tensors"
            )
        return dtype_info
